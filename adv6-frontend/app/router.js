...
import { default as userNavBarMenu } from "./menu-user.json";
...
export const popStateRouter = function (e) {
  ...
  routeTo(document.location.href, {
    pushState: false,
    search: search,
    hash: hash,
    forceReload: true,
...
export const setActiveMenuItemByUrl = (newPathName) => {
  ...
  // excluding any integers
  const user = sessionStorage.getItem("ripencc-workbench-user");
  const menuItems = user == "admin"? adminNavBarMenu.main.concat(adminNavBarMenu.footer) : userNavBarMenu.main.concat(userNavBarMenu.footer);
